...
import { useInView, useAnimatedNumber, useReducedMotion } from "../../hooks/useAnimations";
...
export default function RiskScore({ data }: RiskScoreProps) {
  ...
  const animatedProb = useAnimatedNumber(data.probability, 2000, isInView);
  const reducedMotion = useReducedMotion();
  const color = getRiskColor(data.riskLevel);
  const label = getRiskLabel(data.riskLevel);
  const circumference = 2 * Math.PI * 54;
  const progress = reducedMotion ? data.probability / 100 : animatedProb / 100;
        ...
        <div className="relative w-32 h-32 flex-shrink-0">
          <svg className="w-full h-full transform -rotate-90" viewBox="0 0 120 120">
            ...
            <circle
              ...
              style={{ transition: reducedMotion ? "none" : "stroke-dashoffset 2s ease-out" }}
            />
          </svg>
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span className="text-3xl font-bold text-white">
              {reducedMotion ? data.probability : animatedProb}%
            </span>
          </div>
        </div>
